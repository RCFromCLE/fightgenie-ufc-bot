            ...
            .addFields(
                {
                    ...
                    value: isLifetime
                        ? "• One-time payment for permanent access\n• Server-wide access to all predictions\n• Never pay again!"
                        : `• Access until event completion\n• Event Date: ${event?.[0]?.Date ? new Date(event[0].Date).toLocaleDateString() : 'TBA'}\n• Server-wide access for one event\n• Perfect for watch parties`,
                    inline: false,
                },
                {
                    name: "Next Steps",
      ...
      if (paymentStatus.success) {
        ...
        // Get event info for non-lifetime purchases
          ...
          await database.query(`
              ...
              FROM events e
              WHERE e.Event = 'UFC 310: Pantoja vs. Asakura'
              LIMIT 1
          `) : null;
              ...
              .setTitle("🌐 Complete Your PayPal Purchase")
              .setDescription(
                  `Complete your payment of $${paymentRecord[0].amount.toFixed(2)} through PayPal to activate ${
                      paymentRecord[0].payment_type?.includes('LIFETIME') ? 'lifetime' : 'event'
                  } access.`
              )
.addFields(
    {
  ...
  static async handleSolanaVerification(interaction, paymentId, serverId, amount) {
    try {
        ...
        if (result.success) {
            ...
            const isLifetime = parseFloat(amount) >= 0.5;
                ...
                .setDescription([
                    ...
                    isLifetime ? 
                        ...
                        [
                            ...
                            '',
                            `Event: ${eventInfo?.Event}`,
                            `Date: ${new Date(eventInfo?.Date).toLocaleString()}`,
                            `Access Expires: ${expirationDate.toLocaleString()} EST`,
                        ].join('\n'),
                    '',
                    'You can now use all Fight Genie features:',
                    '• AI-powered fight predictions',
