...
class PaymentModel {
    ...
    static async createServerPayment(serverId, adminId, paymentType, paymentMethod = 'PAYPAL') {
        try {
            console.log('Creating server payment:', { serverId, adminId, paymentType, paymentMethod });
        ...
        try {
            ...
            if (status === 'COMPLETED') {
                ...
                if (payment[0].payment_type === 'SERVER_LIFETIME') {
                    await this.activateServerLifetimeSubscription(payment[0].server_id, paymentId);
    ...
    static async generateSolanaPaymentAddress() {
        try {
            ...
            const merchantWallet = new web3.PublicKey(this.SOLANA_CONFIG.MERCHANT_WALLET);
    ...
    static async updateSolanaPaymentStatus(paymentId, status, transactionSignature = null) {
        try {
            await database.query(`
                UPDATE solana_payments
                SET 
    ...
    static async getServerSubscriptionInfo(serverId) {
        try {
            ...
            console.log('Found subscription info:', subInfo[0] || 'No active subscription');
